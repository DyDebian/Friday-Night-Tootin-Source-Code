...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('showCpuMiddleScroll', ClientPrefs.showCpuMiddleScroll);
		...
		set('shaderEffects', ClientPrefs.shaderEffects);
		...
		Lua_helper.add_callback(lua,"setCamZoom", function(cam:String ,zoomAmount:Float) {
			cameraFromString(cam).zoom = zoomAmount;
		});
		...
		Lua_helper.add_callback(lua, "resetShader", function(type:String) {
			PlayState.instance.resetShader(type);
		});
		// gay ass tweens
		...
		Lua_helper.add_callback(lua, "doTweenScale", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			var obj:Dynamic = tweenShit(tag, vars);
			if(obj != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(obj.scale, {x: value, y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case 'camera' | 'cam': return FlxG.camera;
